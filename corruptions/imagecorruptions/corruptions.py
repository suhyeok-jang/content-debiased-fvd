...
def gauss_function(x, mean, sigma): #Gaussian pdf
...
def _motion_blur(x, radius, sigma, angle):
    width = getOptimalKernelWidth1D(radius, sigma) #2*radius + 1
    kernel = getMotionBlurKernel(width, sigma) 
    #np.deg2rad : angle -> radian
...
def motion_blur(x, severity=1, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    angle = np.random.uniform(-45, 45) #randomness
...
def motion_blur_custom(x, severity=1, seed=None, angle=0):
    shape = np.array(x).shape
    c = [(10, 3), (15, 5), (15, 8), (15, 12), (20, 15)][severity - 1]
    x = np.array(x)
...
def motion_blur_fraction(x, severity=1, seed=None):
    if seed is not None:
        np.random.seed(seed)
        
    shape = np.array(x).shape

    #min : 1단계 절반보다 살짝 아래, max : 2단계
    min_radius, max_radius = 5, 15
    min_sigma, max_sigma = 1, 5

    # severity가 [0, 1] 사이의 연속 값이라고 가정
    radius = min_radius + severity * (max_radius - min_radius)
    sigma = min_sigma + severity * (max_sigma - min_sigma)
    
    x = np.array(x)

    angle = np.random.uniform(-45, 45) #randomness
    x = _motion_blur(x, radius=radius, sigma=sigma, angle=angle)
...
def elastic_transform(image, severity=1, seed = None):
    if seed is not None:
        np.random.seed(seed)
    ...
    #seed 고정을 위해 np.random 한번만 호출
    random_state = np.random.uniform(-1, 1, size=shape[:2])
    dx = (gaussian(random_state * max_dx, sigma, mode='reflect', truncate=3) * alpha).astype(np.float32)
    dy = (gaussian(random_state * max_dy, sigma, mode='reflect', truncate=3) * alpha).astype(np.float32)
...
# mod of https://gist.github.com/erniejunior/601cdf56d2b424757de5
def elastic_transform_fraction(image, severity=1, seed = None):
    if seed is not None:
        np.random.seed(seed)
    ...
    min_alpha, max_alpha = 250*0.02, 250 * 0.065  #min: 기존 1의 절반보다 살짝 아래, max :2
    max_dx = shape[0] * 0.005
    max_dy = shape[0] * 0.005
    
    alpha = min_alpha + severity * (max_alpha - min_alpha) #severity: [0-1]
