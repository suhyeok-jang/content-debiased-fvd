...
corruption_tuple = (gaussian_noise, shot_noise, impulse_noise, defocus_blur,
                    glass_blur, motion_blur, motion_blur_custom, motion_blur_fraction, zoom_blur, snow, frost, fog,
                    brightness, contrast, elastic_transform, elastic_transform_fraction, pixelate,
...
def corrupt(image, severity=1, corruption_name=None, corruption_number=-1, seed = None, angle = None):
    ...
    if not (corruption_name is None):
        if corruption_name == "motion_blur_custom": #angle도 필요
            image_corrupted = corruption_dict[corruption_name](Image.fromarray(image),
                                                severity, seed=seed, angle=angle)
        else:
            image_corrupted = corruption_dict[corruption_name](Image.fromarray(image),
                                                        severity, seed=seed)
