...
from .third_party.VideoMAEv2.utils_copy import load_videomae_model, preprocess_videomae
...
def get_videomae_features(stats, model, videos, batchsize=16, device='cuda', model_dtype=torch.float32, concat = False):
...
class cdfvd(object):
    ...
    def compute_real_stats(self, loader: Union[torch.utils.data.DataLoader, List, None] = None, concat = False, make_gif = False) -> FeatureStats:
        '''
        ...
        while self.real_stats.max_items is None or self.real_stats.num_items < self.real_stats.max_items:
            for i, batch in enumerate(tqdm(loader)):
                ...
                self.real_stats = self.feature_fn(self.real_stats, self.model, real_videos, device=self.device, model_dtype=self.model_dtype, concat = concat)
    ...
    def compute_fake_stats(self, loader: Union[torch.utils.data.DataLoader, List, None] = None, concat = False, make_gif = False) -> FeatureStats:
        '''
        ...
        while self.fake_stats.max_items is None or self.fake_stats.num_items < self.fake_stats.max_items:
            for i,batch in enumerate(tqdm(loader)):
                ...
                self.fake_stats = self.feature_fn(self.fake_stats, self.model, fake_videos, device=self.device, model_dtype=self.model_dtype, concat=concat)
